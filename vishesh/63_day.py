...
# --- 4. Backtesting Strategy (SMA Trend Filter + 63d VIX Ratio) ---

print("\nRunning Backtest: SMA 200 Trend Filter + 63d VIX Ratio Strategy")
...
calculate_metrics(data['Strategy_Returns'], name="Trend Filter + 63d VIX Strategy")
...
# --- Plot 1: Trend Filter 63d Time Series Plots (4 panels) ---
...
# Panel 2: VIX Ratio and 63d Bands
...
axes[1].plot(data.index, data['q_50_rolling'], color='blue', linestyle='--', label=f'Rolling 63d 50th Quantile - "Re-Buy Signal"')
axes[1].set_title('VVIX/VIX Ratio and DYNAMIC 63-day Bands', fontsize=16)
axes[1].legend(loc='upper left')

# Panel 3: Position of VIX Logic
axes[2].plot(data.index, data['position_vix'], label='VIX Position (1=In, 0=Out)', color='grey')
axes[2].set_title('VIX-Only Strategy Position (63-day window)', fontsize=16)
axes[2].set_yticks([0, 1])
axes[2].legend(loc='upper left')

# Panel 4: Final Combined Position
axes[3].plot(data.index, data['position'], label='Final Position (1=In, 0=Out)', color='blue')
axes[3].set_title('Final Strategy Position (Trend Filter + 63d VIX)', fontsize=16)
axes[3].set_yticks([0, 1])
axes[3].set_xlabel('Date')
axes[3].legend(loc='upper left')

plt.tight_layout()
save_path = os.path.join(plot_dir, 'trend_filter_63d_spy_and_ratio.png')
plt.savefig(save_path)
plt.close()
print(f"Trend Filter 63d time series plots saved as '{save_path}'")

# --- Plot 2: Trend Filter 63d Strategy PnL vs. Buy & Hold ---

data['Strategy_Cumulative'] = (1 + data['Strategy_Returns']).cumprod()
data['Buy_Hold_Cumulative'] = (1 + data['SPY_Returns']).cumprod()

plt.figure(figsize=(15, 8))
plt.plot(data['Strategy_Cumulative'], label='Trend Filter + 63d VIX Strategy', color='blue')
plt.plot(data['Buy_Hold_Cumulative'], label='SPY Buy & Hold', color='orange', alpha=0.8)

plt.title('Trend Filter + 63d VIX Strategy vs. Buy & Hold', fontsize=16)
plt.ylabel('Cumulative Returns')
plt.xlabel('Date')
plt.legend(loc='upper left')
plt.yscale('log')
    
save_path = os.path.join(plot_dir, 'trend_filter_63d_strategy_vs_buy_hold.png')
plt.savefig(save_path)
plt.close()
print(f"Trend Filter 63d Strategy PnL plot saved as '{save_path}'")

# --- 7. Save "Diff File" ---
diff_file_path = os.path.join(plot_dir, 'trend_filter_63d_gemini_backtest_data.csv')
data.to_csv(diff_file_path)
print(f"Trend Filter 63d Backtest data 'diff file' saved as '{diff_file_path}'")
