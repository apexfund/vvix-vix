...
# --- 4. Backtesting Strategy (INVERTED "Hold by Default") ---

print("\nRunning Backtest: Inverted 'Hold by Default' Strategy")

# Create signals: -1 = Sell, 1 = Buy, 0 = Hold
data['signal'] = 0
# SHIFT(1) is critical to avoid lookahead bias
# SELL Signal (Complacency)
data.loc[data['VVIX_VIX_Ratio'].shift(1) >= data['q_95_rolling'].shift(1), 'signal'] = -1
# RE-BUY Signal (Return to Median)
data.loc[data['VVIX_VIX_Ratio'].shift(1) <= data['q_50_rolling'].shift(1), 'signal'] = 1

# Create position: 1 = In Market, 0 = Out of Market
data['signal_temp'] = data['signal'].replace(0, np.nan)

# *** KEY CHANGE: .fillna(1) to start IN the market ***
data['position'] = data['signal_temp'].ffill().fillna(1)

# Clean up: Replace -1 (sell) with 0 (cash)
...
calculate_metrics(data['Strategy_Returns'], name="INVERTED VVIX/VIX Strategy")
...
# --- Plot 1: INVERTED Time Series Plots ---
...
plt.tight_layout()
save_path = os.path.join(plot_dir, 'inverted_spy_and_ratio.png')
plt.savefig(save_path)
plt.close()
print(f"INVERTED time series plots saved as '{save_path}'")

# --- Plot 2: INVERTED Strategy PnL vs. Buy & Hold ---

data['Strategy_Cumulative'] = (1 + data['Strategy_Returns']).cumprod()
data['Buy_Hold_Cumulative'] = (1 + data['SPY_Returns']).cumprod()

plt.figure(figsize=(15, 8))
plt.plot(data['Strategy_Cumulative'], label='INVERTED VVIX/VIX Strategy', color='blue')
plt.plot(data['Buy_Hold_Cumulative'], label='SPY Buy & Hold', color='orange', alpha=0.8)

plt.title('INVERTED Strategy vs. Buy & Hold Cumulative Returns', fontsize=16)
plt.ylabel('Cumulative Returns')
plt.xlabel('Date')
plt.legend(loc='upper left')
plt.yscale('log')
    
save_path = os.path.join(plot_dir, 'inverted_strategy_vs_buy_hold.png')
plt.savefig(save_path)
plt.close()
print(f"INVERTED Strategy PnL plot saved as '{save_path}'")

# --- 7. Save "Diff File" ---
diff_file_path = os.path.join(plot_dir, 'inverted_gemini_backtest_data.csv')
data.to_csv(diff_file_path)
print(f"INVERTED Backtest data 'diff file' saved as '{diff_file_path}'")
